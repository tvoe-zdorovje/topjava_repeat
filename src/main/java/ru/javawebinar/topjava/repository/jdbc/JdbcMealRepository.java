...
public abstract class JdbcMealRepository {
    ...
    public Meal save(Meal meal, SqlParameterSource sqlParameterSource) {
        if (meal.isNew()) {
            Number newId = insertMeal.executeAndReturnKey(sqlParameterSource);
            meal.setId(newId.intValue());
        } else {
            if (namedParameterJdbcTemplate.update("" +
                            "UPDATE meals " +
                            "   SET description=:description, calories=:calories, date_time=:date_time " +
                            " WHERE id=:id AND user_id=:user_id", sqlParameterSource) == 0) {
                return null;
            }
        }
        return meal;
    ...
    @Profile("hsqldb")
    @Repository
    public static class HqlDbJdbcMealRepository extends JdbcMealRepository implements MealRepository {

        public HqlDbJdbcMealRepository(JdbcTemplate jdbcTemplate, NamedParameterJdbcTemplate namedParameterJdbcTemplate) {
        ...
        public Meal save(Meal meal, int userId) {
            ...
            return super.save(meal, sqlParameterSource);
        }
